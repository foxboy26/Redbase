...
PF_BufferPool::PF_BufferPage::PF_BufferPage()
    : isDirty(false), pinCount(0), refBit(0) {
...
PF_BufferPool::PF_BufferPool(int bufferSize) : bufferSize_(bufferSize) {
...
//
//     auto key =
//         std::pair<int, PageNum>(bufferPool[slot].fd,
//         bufferPool[slot].pageNum);
//     indexMap.erase(key);
//
